...
# Description: Test for SIMQASL BERT
...
def model():
    ...
    model = BertQASLPredictor(
        model_dir=model_dir, query_path=query_path, with_bilstmcrf=True
    )
    return model
...
def run_flat_test(model, testcase):
    ...
    ents = model.predict_overall(passage)  ## List[NamedTuple]
...
@pytest.mark.xfail(reason="DataBug: Controversial Answers")
def test_flat_10(model, testcase_flat_10):
    run_flat_test(model, testcase_flat_10)


# --- TEST FOR NESTED NER ---
@pytest.mark.xfail(reason="ModelBug: It's an old trained model. Haven't changed.")
